...
x <- c("Int", "RVImax","SLmax","SLmin","WL","CC","Block", "P4", "P5", "P6", "ET4", "ET5", "ET6")
args_unique <- list(x)
...
args_in_common <- list(y = "Yield",             # Defining dependent variable for the procedure
                       ...
                       nfolds = 10,             # Number of foldings, i.e. split between training and test data
...
# This is necessary if you want to calculate standard deviation for MAE based on the 10 different foldings (developed models).
...
# There is a leaderboard of models in the h2o.aml object.
# One of best models is a gradient boosting machine
# The variable importance plot was created by running h2o.varimp() on this gradient boosting machine object.
...
outdata02 = h2o.cbind(indata, glm_cvpreds, aml_cvpreds) %>%
  as.data.frame()
outdata02 %>%
  write_csv(here("data","processed","Outdata02.csv"))
